<template>
  ...
  <pokemon-item-evolution-arrow v-if="evolutionChain.evolves_to.length === 1" :position="position"/>

  <div v-if="evolutionChain.evolves_to.length === 2"
       :style="{'grid-area': 'arrow-' + computePositionArrow(1, depth)}"
       class="d-flex justify-content-between flex-column">
    <pokemon-item-evolution-arrow :rotation="-45"/>
    <pokemon-item-evolution-arrow :rotation="45"/>
  </div>
  ...
  <div v-if="evolutionChain.evolves_to.length === 3"
       ...
       class="d-flex justify-content-between flex-column">
    <pokemon-item-evolution-arrow :rotation="-45"/>
    <pokemon-item-evolution-arrow />
    <pokemon-item-evolution-arrow :rotation="45"/>
  </div>

  <template v-if="evolutionChain.evolves_to.length < 8">
    <pokemon-item-evolution-node v-for="(next, index) in evolutionChain.evolves_to"
                                   :key="index"
  ...
  <template v-else>
    <pokemon-item-evolution-node v-for="(pos, index) in ['0-0', '0-1', '0-2', '1-0', '1-2', '2-0', '2-1', '2-2']"
                                   :key="index"
                                   :evolution-chain="evolutionChain.evolves_to[index]"
                                   :position="pos"/>
    <pokemon-item-evolution-arrow v-for="(element, index) in [{pos: '0-0', rotation: 225}, {pos: '0-1', rotation: 270},
...
export default {
  ...
  components: {PokemonItemEvolutionArrow, PokemonCard},
  props: {
    evolutionChain: {
      type: Object,
      required: true,
